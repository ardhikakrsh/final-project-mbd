        ...
        <div class="col-xl-4 col-lg-5 col-md-7 mx-auto">
            <div class="card z-index-0">
                <div class="card-header text-center bg-gradient-primary">
                    <h5 class="text-white m-0">Edit Pembayaran {{$pembayaran->sewa->pelanggan->user->name}} kendaraan {{$pembayaran->sewa->kendaraan->nama}}</h5>
                </div>
                <div class="card-body">
                    <form role="form text-left" method="POST" action="{{ route('admin.pembayarans.update', $pembayaran) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        {{-- <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="metode_pembayaran" class="form-control" placeholder="Metode Pembayaran" name="metode_pembayaran" id="metode_pembayaran"
                                aria-label="Metode Pembayaran" aria-describedby="Metode Pembayaran" value="{{ old('metode_pembayaran',$pembayaran->metode_pembayaran) }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" placeholder="0" name="jumlah_pembayaran"
                                ...
                                aria-describedby="Jumlah Pembayaran" value="{{ old('jumlah_pembayaran',$pembayaran->jumlah_pembayaran) }}"
                                required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="choices form-select" name="detail_pembayaran" id="detail_pembayaran" required>
                                ...
                                @foreach ($types as $type)
                                    <option value="{{ $type }}"  @if (old('detail_pembayaran',$pembayaran->detail_pembayaran) == $type ) selected @endif>{{ $type }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            @if ($pembayaran->bukti_pembayaran)
                                <div class="card shadow-none mb-1">
                                    <img src="{{ asset('assets/img/payment-proof/' . $pembayaran->bukti_pembayaran) }}" alt="Bukti: {{$pembayaran->bukti_pembayaran}}" class="card-img">
                                </div>
                            @endif
                            <input type="file" class="form-control" name="bukti_pembayaran"
                                id="bukti_pembayaran" aria-label="Bukti Pembayaran"
                                aria-describedby="Bukti Pembayaran" value="{{ old('bukti_pembayaran', $pembayaran->bukti_pembayaran) }}"
                                >
                        ...
                        <div class="mb-3">
                            ...
                            <select class="choices form-select"
                                ...
                                @foreach ($status as $item)
                                    <option value="{{ $item }}"  @if (old('status_pembayaran', $pembayaran->status_pembayaran) == $item ) selected @endif>{{ $item }}</option>
                                @endforeach
                            </select>
                        ...
                        <input type="hidden" name="sewa_id" value="{{$pembayaran->sewa_id}}">
                        <div class="row">
